...
export function SettlementTab({ sessionId, session, teams, matches, attendance, onRefetch }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* 납부 요약 카드 */}
      <div className="bg-gradient-to-br from-primary/10 to-teal-50 dark:from-primary/20 dark:to-teal-500/10 rounded-2xl p-6 border border-primary/30">
      ...
      {bankAccount && bankAccount.accountNumber && (
        <div className="bg-white dark:bg-slate-900/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            >
              {copiedBank ? <Check className="w-3.5 h-3.5 text-primary" /> : <Copy className="w-3.5 h-3.5" />}
              {copiedBank ? '복사됨' : '복사'}
            </button>
          </div>
        </div>
          ...
          {(summary.totalAmount > 0 || expenses.length > 0) && (
            <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 space-y-1.5">
              ...
              <div className="flex justify-between text-sm font-semibold pt-1.5 border-t border-slate-100 dark:border-slate-800">
                ...
                <span className={cn(
                  (summary.netRevenue ?? 0) >= 0 ? 'text-primary' : 'text-red-500'
                )}>
      ...
      {session.status === 'completed' && (
        <div className="bg-primary/5 rounded-2xl p-6 border border-primary/30 text-center">
          <div className="w-12 h-12 bg-primary rounded-full flex items-center justify-center mx-auto mb-3">
            <Check className="w-6 h-6 text-white" />
          </div>
          <h4 className="font-semibold text-primary">정산 완료</h4>
