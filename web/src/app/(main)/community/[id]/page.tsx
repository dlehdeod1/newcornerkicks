...
const categoryColorMap: Record<string, { badge: string; text: string }> = {
  emerald: { badge: 'bg-primary/10', text: 'text-primary' },
  ...
  if (!post) {
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        <Link href="/community" className="text-sm text-primary mt-4 inline-block">
          목록으로 돌아가기
        </Link>
      </div>
    )
      ...
      <article className="bg-white/80 dark:bg-slate-900/50 backdrop-blur rounded-2xl p-6 border border-slate-200 dark:border-slate-800/50 shadow-sm mb-6">
        ...
        {isRecruitOrMatch && (
          <div className="flex flex-wrap gap-2 mb-5 pb-5 border-b border-slate-100 dark:border-slate-800">
            ...
            {post.headcount && (
              <span className="px-3 py-1 text-sm bg-primary/10 text-primary rounded-lg font-medium">
                {post.headcount}명 모집
              </span>
            )}
          </div>
      ...
      <div className="bg-white/80 dark:bg-slate-900/50 backdrop-blur rounded-2xl p-6 border border-slate-200 dark:border-slate-800/50 shadow-sm">
        ...
        <form onSubmit={handleSubmitComment} className="flex gap-2 mb-6">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/50"
          />
          <button
            type="submit"
            disabled={!comment.trim() || addCommentMutation.isPending}
            className="px-4 py-2.5 bg-primary hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl text-sm font-medium transition-colors"
          >
